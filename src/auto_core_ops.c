...
// Create an automaton with sharp transistions where replace occurs once
// on a matched string.
// To do this create *three* copies of the DFA
//
// 1) The original states (before replace)
// 2) The sharp set of states for replacement (which can be transitioned to from 1)
// 3) The final states which occur after replacement (which can be transitioned to from 2)
//
DFA *dfa_replace_once_step1_duplicate(DFA *M, int var, int *indices) {
  ...
  newns = 3 * (M->ns - 1) + 1; //number of states after duplicate. The sink state is not duplicated.
  ...
  char exeps[max_exeps][len + 1];
  char to_states[max_exeps];
  char statuces[newns];
    ...
    if (i != sink) {
      ...
      while (pp) {
        if (pp->to != sink) {
          to_states[k] = pp->to;
          for (j = 0; j < var; j++) {
            //the following for loop can be avoided if you take a walk through what is known as....
            for (tp = pp->trace; tp && (tp->index != indices[j]); tp = tp->next);
            if (tp) {
              if (tp->value)
                exeps[k][j] = '1';
      ...
      for (k--; k >= 0; k--)
        dfaStoreException(to_states[k], &exeps[k][0]);
      if (i > sink)
  ...
  for (i = 0; i < M->ns; i++) {
    if (i != sink) {
      ...
      for (k--; k >= 0; k--)
        dfaStoreException(to_states[k], &exeps[k][0]);
      // Add transitions to the third set of states
      if (i > sink) {
          dfaStoreException(i + (shift - 1) * 2, sharp0);
      } else {
          dfaStoreException(i + (shift * 2) - 1 , sharp0);
      }
      dfaStoreState(sink);
  ...
  for (i = 0; i < M->ns; i++) {
    if (i != sink) {
      ...
      while (pp) {
        if ((pp->to) != sink) {
          if ((pp->to) > sink) {
            to_states[k] = pp->to + (shift * 2) - 2; // to new state, sink state will be eliminated and hence need -1
          } else {
            to_states[k] = pp->to + (shift * 2) - 1; // to new state
          }
          for (j = 0; j < var; j++) {
      ...
      // Default state always sink
      dfaStoreState(sink);
      duplicate_id = i + (shift * 2) - 1;
      if (i > sink) {
        duplicate_id--;
      }
      if (M->f[i] == 1)
        ...
        statuces[duplicate_id] = '0';
      kill_paths(state_paths);
    } else if (sink < M->ns - 1) { // Sink state i == sink
      if (M->f[sink + 1] == -1) {
        statuces[sink + shift * 2 - 1] = '-';
      } else {
        statuces[sink + shift * 2 - 1] = '0';
      }
    }
  }
  statuces[newns] = '\0';
